...
int convert_CP1251_to_UTF8(FILE *stream, FILE *stream_save)
{
    ...
    while (!feof(stream) && !ferror(stream))
    {
        ...
        {
            int idx = idxs[char1251-128];
            int len = lens[char1251-128];
...
int convert_KOI8_R_to_UTF8(FILE *stream, FILE *stream_save)
{
    ...
    while (!feof(stream) && !ferror(stream))
    {
        fread(&charKoi8, sizeof(char), 1, stream);
        if(charKoi8 <= 127)
        {
            write_[0] = charKoi8;
            fwrite(write_, 1 , sizeof(write_), stream_save);
        ...
        {
            int idx = idxs[charKoi8-128];
            int len = lens[charKoi8-128];
...
int convert_ISO_8859_5_to_UTF8(FILE *stream, FILE *stream_save)
{
    ...
    while (!feof(stream) && !ferror(stream))
    {
        fread(&charIso_8859_5, sizeof(char), 1, stream);

        if(charIso_8859_5 <= 127)
        {
            write_[0] = charIso_8859_5;
            fwrite(write_, 1 , sizeof(write_), stream_save);
        }
        else
        {
            int idx = idxs[charIso_8859_5-128];
            int len = lens[charIso_8859_5-128];
